...
public class SingleTurtle extends Turtle{
	...
	protected SingleTurtle getCopy() {
		...
		turtle.setBeenStamped(myPen.hasBeenStamped());
		return turtle;
	}
	
	private void setBeenStamped(boolean hasBeenStamped) {
		myPen.setBeenStamped(hasBeenStamped);
	}
	...
	private class Pen {		
		...
		private Pen(Group penLines) {
			this(penLines, true, DEFAULT_PEN_COLORCODE, DEFAULT_PEN_WIDTH, false, new ArrayList<ImageView>());
		}

		private Pen(Group penLines, String colorCode) {
			this(penLines, true, colorCode, DEFAULT_PEN_WIDTH, false, new ArrayList<ImageView>());
		}
		...
		private Pen(Group penLines, String colorCode, double width) {
			this(penLines, true, colorCode, width, false, new ArrayList<ImageView>());
		}
		 ...
		 * @param width is initial width of Pen
		 */
		private Pen(Group penlines, boolean down, String colorCode, double width, boolean stamped, List<ImageView> stamps) {
			myPenLines = penlines;
			myColorCode = colorCode;
			myIsDown = down;
			myWidth = width;
			myBeenStamped = stamped; 
			myStamps = stamps; 
		}
		...
		private void stamp() {
			...
			stamp.setFitWidth(DEFAULT_TURTLE_SIZE);
			stamp.setFitHeight(DEFAULT_TURTLE_SIZE);
			stamp.setX(myX - DEFAULT_TURTLE_SIZE/2);
			stamp.setY(myY - DEFAULT_TURTLE_SIZE/2);
			myStamps.add(stamp);
			myPenLines.getChildren().add(stamp);
		}

		private void clearStamps() {
			myBeenStamped = false; 
			myPenLines.getChildren().removeAll(myStamps);
		}
		
		private void setBeenStamped(boolean stamped) {
			myBeenStamped = stamped; 
		}
		
		private boolean hasBeenStamped() {
			return myBeenStamped; 
		}
